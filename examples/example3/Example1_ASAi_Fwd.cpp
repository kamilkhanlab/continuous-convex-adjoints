...
#define NX       2             /* number of state variables */
...
double pL[NP] = { -6.5, 0.01 }; /* lower bound of parameters */
double pU[NP] = { 6.5,  0.5 };    /* upper bound of parameters */
double fixedp[NP] = { -3.0, 0.5};    /* values of parameters      */
...
template <typename T, typename U> T Original_RHS(T x[NX], U p[NP], int n)
{
    ...
    {
    case 0:
        f_rhs = -1 * (2 + sin(p[0] / 3)) * pow(x[0], 2) + p[1] * x[0] * x[1];
        break;

    case 1:
        f_rhs = sin(p[0] / 3) * pow(x[0], 2) - p[1] * (x[0] * x[1]);
        break;
    }
 ...
 * fB is the RHS function for the ODE system which is to solve lambda.
 * fB = lambda^T * df/dx
*/
...
static int fQB(realtype t, N_Vector x, N_Vector xB, N_Vector qBdot, void* user_dataB)
{
    ...
    /* Initialize subgradients for pMCsub with respect to p*/
...
static void PrintOutput(realtype tfinal, N_Vector x_Re, N_Vector xB, N_Vector qB)
{
    ...
    printf("x(t0):      %12.4e %12.4e \n",
        Ith(x_Re, 3), Ith(x_Re, 4));
    printf("--------------------------------------------------------\n");
    printf("Sensitivity       p1           p2\n\n");
